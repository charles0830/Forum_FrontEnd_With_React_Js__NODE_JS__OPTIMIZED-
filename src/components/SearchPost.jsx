...
export class SearchPost extends Component {
    ...
    donereport=()=>{
     ...
     componentDidMount(){
        const {posttype} = this.props.match.params;
            ...
            axios.get(`/searchpost/${localStorage.getItem('serial')}/${posttype}`,{
                headers: {
                    'Content-Type' : 'application/json'
                }
            }).then(response=>{
                this.setState({topicpost : response.data.searchResult})
            }).catch(error=>{
                console.log(error)
            })
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (posttype !== prevPostType) {
            axios.get(`/searchpost/${this.props.sln}/${posttype}`,{
                headers: {
                    'Content-Type' : 'application/json'
                }
            }).then(response=>{
