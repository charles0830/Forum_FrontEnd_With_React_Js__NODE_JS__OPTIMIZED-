    ...
    mypostdata = ()=>{
        if(this.state.posts){
            return  this.state.posts.map((each)=>{
                ...
                <div className='row bodbot p-2'>   
                    <div className='col d-flex justify-content-start flex-column flex-md-row text-break'><div className='imgbox me-3'><img className='imgfit' src={each.image}></img></div><span><span className='postbold'>{each.author}</span><br/>posted at : {each.creating_time}</span></div>
                </div>
                <div className='row mt-3 p-2'>
                    <div className='col d-flex justify-content-start flex-column flex-md-row text-break'>
            ...
            <div className='reactbox postbox2 mt-2 mx-auto likepad'>
                <div className='row row-cols-12 d-flex flex-row justify-content-center align-items-bottom'>
                    <div className='col-4 d-flex justify-content-center align-items-center'> <p><i className="fa-solid fa-thumbs-up bigi"></i>&nbsp;{each.like_amount>100000 ? Math.floor(each.like_amount/100000) +'M': each.like_amount>1000 ? Math.floor(each.like_amount/1000) +'K' : each.like_amount}</p></div>
                    <div className='col-3 d-flex justify-content-center align-items-center'><p><i className="fa-solid fa-thumbs-down bigi"></i> {each.dislike_amount>100000 ? Math.floor(each.dislike_amount/100000) +'M': each.dislike_amount>1000 ? Math.floor(each.dislike_amount/1000) +'K' : each.dislike_amount}</p></div>
                    <div className='col-3 d-flex justify-content-center align-items-center'><Link className='linc2' to={'/posteditmain/'+each.slno}><p><i className="fa-solid fa-pen-to-square bigi linkdec3"></i></p></Link></div>
                </div>
            </div></div>
